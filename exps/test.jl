...
function batch_eval(
    ...
    C, As, bs = input(A)
    res = sdplr(C, As, bs, r; prior_trace_bound=Float64(trace_bound), dataset=graph, kwargs...)
...
graph = args["graph"]
ind = findfirst(x -> x == problem, ["MaxCut", "MinimumBisection", "LovaszTheta", "CutNorm"])
...
trace_bounds = [n, n, 1, n] 
# run the benchmark
batch_eval(problem, graph, A, inputs[ind], callbacks[ind],  
         "SDPLR-R-$(args["rank"])-seed-$seed-tol-$tol", trace_bounds[ind], args["rank"]; 
         maxtime=36000.0, objtol=args["objtol"], ptol=args["ptol"])