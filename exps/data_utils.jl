...
"""
function read_graph(
    filename::String; filefolder::String=homedir()*"/datasets/graphs/"
)
...
function write_matrix_sdplr(
    A::Union{SparseMatrixCSC{Tv,Ti},SparseMatrixCOO{Tv,Ti}}, id::Ti, f::IOStream
...
function write_matrix_sdplr(
    A::SymLowRankMatrix{Tv}, id::Ti, f::IOStream
) where {Tv<:AbstractFloat,Ti<:Integer}
...
function write_matrix_sdplr(
    A::AbstractMatrix{Tv}, id::Ti, f::IOStream
) where {Tv<:AbstractFloat,Ti<:Integer}
...
function gset_sdp_preprocess_data(
    ...
    for i in [1:67; 70; 72; 77; 81]
        ...
        if sdp_format == "sdpa"
            write_problem_sdpa(
                "G$i."*sdp_format, C, As, bs; filefolder=output_folder
            )
